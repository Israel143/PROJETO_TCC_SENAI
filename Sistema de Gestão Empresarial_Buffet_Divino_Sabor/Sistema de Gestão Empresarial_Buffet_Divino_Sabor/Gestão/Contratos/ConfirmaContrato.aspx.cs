...
{
    public partial class ConfirmaContrato : System.Web.UI.Page
    {
        ...
        protected void btnAceitarContrato_Click(object sender, EventArgs e)
        {
            var pedidoID = Session["pedidoID"].ToString();
            var contrato = new Classe.contratos();
            var cadastro = new Negocio.contratos().Create(pedidoID);

            if(cadastro == true)
            {
                var pedido = new Classe.pedidos();
                pedido.id = Convert.ToInt32(pedidoID);
                ...
                if(pedidos == true)
                {
                    SiteMaster.ExibirAlertRedirecionar(this, "Contrato efetuado com sucesso!", "../Pedidos/Pedidos.aspx");
                }
